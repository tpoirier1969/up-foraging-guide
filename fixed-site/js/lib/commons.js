...
function significantTokens(text) {
  const stop = new Set(["the","and","for","with","from","into","only","very","common","wild","yellow","black","red","american","michigan","upper"]);
...
async function mediaWikiSearch(query) {
  ...
  url.searchParams.set("gsrlimit", String(Math.max(IMAGE_SEARCH_LIMIT, 12)));
...
function exactSpeciesMatch(candidate, record) {
  ...
  const tokens = significantTokens(common).slice(0, 5);
