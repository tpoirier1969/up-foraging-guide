import { loadLocalData, loadSupabaseData, loadOverridePayload } from "./api-wm4.js?v=v2.0-wm4";
import { sortRecords, normalizeRecord, isPlant, isForagingMushroom, medicinalRecords, reviewRecords, avoidRecords } from "./data-model-wm4.js?v=v2.0-wm4";
import { state } from "./state.js?v=v2.0";
import { parseRoute } from "./router.js?v=v2.0";
import { MONTHS } from "./constants-wm4.js?v=v2.0-wm4";
import { renderDashboard } from "./pages-wm4.js?v=v2.0-wm4";
import { updateHeaderStats, renderPage, markActiveNav, bindDetailLinks, bindSharedActions, wireModal, openDetail } from "./ui-wm4.js?v=v2.0-wm4";
...
const filterState = { home: emptyFilter('home'), plants: emptyFilter(), mushrooms: emptyFilter(), medicinal: emptyFilter(), lookalikes: emptyFilter(), review: emptyFilter(), credits: emptyFilter() };
const paneMode = { home: 'results', plants: 'results', mushrooms: 'results', medicinal: 'results', lookalikes: 'results', timeline: 'results', review: 'results', credits: 'results' };
...
function renderCurrentRoute() {
  ...
  const allowedPages = ['home','plants','mushrooms','medicinal','lookalikes','timeline','review','credits'];
  ...
  if (route.page === 'detail' && route.slug) {
    if (!document.getElementById('pageRoot').innerHTML.trim()) {
      renderPage(renderDashboard({ page: activePage, allRecords: state.allRecords, currentRecords: filteredForPage(activePage), filters: filterState[activePage] || emptyFilter(activePage), selectedMonth: selectedTimelineMonth, selectedWeek: selectedTimelineWeek, paneMode: paneMode[activePage] || 'results', overridePayload }));
    }
    bindDetailLinks();
  ...
  bindDetailLinks();
  bindSharedActions({
    onFilterChange: event => { const target = event.currentTarget; const page = state.route; if (!filterState[page]) return; filterState[page][target.dataset.filter] = target.value; if (target.dataset.filter === 'treeType') filterState[page].hostTree = ''; renderCurrentRoute(); },
    onClearFilters: () => { const page = state.route; if (!filterState[page]) return; filterState[page] = emptyFilter(page); renderCurrentRoute(); },
    onTimelineMonthChange: (month, week) => { if (!month) return; selectedTimelineMonth = month; selectedTimelineWeek = Number(week || 1); renderCurrentRoute(); },
    onPaneModeChange: (page, nextPaneMode) => { if (!page || !paneMode[page] || !nextPaneMode) return; paneMode[page] = nextPaneMode; renderCurrentRoute(); },
    onTimelineShift: direction => { const index = MONTHS.indexOf(selectedTimelineMonth); if (index < 0) return; const delta = direction === 'prev' ? -1 : 1; const nextIndex = (index + delta + MONTHS.length) % MONTHS.length; selectedTimelineMonth = MONTHS[nextIndex]; renderCurrentRoute(); }
  });
}
async function init() {
  wireModal();
