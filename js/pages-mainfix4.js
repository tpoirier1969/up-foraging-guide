...
import { renderResultCard } from "./renderers/cards-mainfix.js?v=2026-04-15-2";
...
function formatLabelFromSlug(slug) {
  return String(slug || "").replace(/-/g, " ").replace(/\w/g, (ch) => ch.toUpperCase());
}
...
function identificationHub() {
  return `
    <section class="panel home-hub">
      <div class="result-header compact-result-header"><div class="result-title-row"><h3>Identification</h3><p class="results-meta">Split by type</p></div></div>
      <div class="home-card-grid">
        <a class="home-card" href="#/plants"><strong>Identify plants</strong><span>Habitat, part, flower color, leaf arrangement, leaf shape, stem surface, taste, and season.</span></a>
        <a class="home-card" href="#/mushrooms"><strong>Identify mushrooms</strong><span>Substrate, tree type, host tree, ring, texture, smell, staining, and season.</span></a>
      </div>
    </section>`;
}
function mushroomStartHere() {
  ...
  return `
    <section class="panel home-hub">
      <div class="result-header compact-result-header"><div class="result-title-row"><h3>Start here</h3><p class="results-meta">Narrow mushrooms fast</p></div></div>
      <div class="home-card-grid">
        <a class="home-card" href="#/identification"><strong>Identification hub</strong><span>Start from scratch if you are not sure what you have.</span></a>
        <a class="home-card" href="#/lookalikes"><strong>Non-edible / caution</strong><span>Check dangerous species before trusting a match.</span></a>
        <a class="home-card" href="#/references"><strong>References</strong><span>Safety, toxicity, handling, and usage notes.</span></a>
      </div>
      <section class="panel" style="margin-top:14px">
        <div class="result-header compact-result-header"><div class="result-title-row"><h3>Best narrowing order</h3><p class="results-meta">Use the strongest traits first</p></div></div>
        <div class="tag-row">
          <span class="tag">1. Month: ${escapeHtml(month)}</span>
          <span class="tag">2. Substrate</span>
          <span class="tag">3. Tree type / host</span>
...
function sortFilter(current) {
  return `<label class="compact-filter"><span>Sort</span><select data-filter="sort">${SORT_OPTIONS.map((opt) => `<option value="${escapeHtml(opt.value)}" ${current === opt.value ? "selected" : ""}>${escapeHtml(opt.label)}</option>`).join("")}</select></label>`;
}
...
function resultSection(title, records, context, filters = {}, metaText = "") {
  const finalMeta = metaText || `${records.length} match${records.length === 1 ? "" : "es"}${filters.sort ? ` · ${escapeHtml(SORT_OPTIONS.find((opt)=>opt.value===filters.sort)?.label || "")}` : ""}`;
