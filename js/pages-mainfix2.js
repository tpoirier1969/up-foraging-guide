import { ACTIVE_NOW_LABEL, MONTHS } from "./constants-mainfix.js?v=v2.1-mainfix";
...
function categoryOptions(records, current) { const categories = [...new Set((records || []).map(record => record.category).filter(Boolean))].sort((a, b) => a.localeCompare(b)); return optionHtml(categories, current, 'Any category'); }
...
function renderResultList(records, context = 'general') { return records.length ? records.map(record => renderResultCard(record, context)).join("") : '<div class="panel empty-state"><h3>No matches</h3><p>All filter options stay visible on purpose now, even when a combination catches nothing.</p></div>'; }
...
function renderFilterForm({ page, filters, extraFilters = [], scopeRecords, title }) {
  ...
  return `<section class="panel workspace-pane filter-pane-card"><div class="section-heading-block"><h3>${escapeHtml(title)} filters</h3><p>All options stay visible all the time. Zero results is allowed. That is a field guide, not a disappearing-act dropdown.</p></div><div class="filter-form-grid"><label><span>Search</span><input type="search" data-filter="search" value="${escapeHtml(filters.search || '')}" placeholder="Search names, Latin names, notes, uses"></label><label><span>Month</span><select data-filter="month">${optionHtml(MONTHS, filters.month, 'Any month')}</select></label>${page === 'home' ? `<label><span>Category</span><select data-filter="category">${categoryOptions(scopeRecords, filters.category)}</select></label>` : ''}${extraFilters.join('')}</div><div class="workspace-actions"><button type="button" data-action="clear-filters">Clear filters</button>${showSeasonButton(page) ? `<a class="buttonish" href="${inSeasonHref}">${ACTIVE_NOW_LABEL}</a>` : ''}<a class="buttonish" href="#/review">Needs Review</a></div><p class="filter-note">Timeline week detail still defaults to week 1 until specific week data gets verified.</p></section>`;
}
function renderWorkspace({ page, title, intro, records, filters, context = 'general', extraFilters = [], paneMode = 'results', scopeRecords }) {
  ...
  const inSeasonHref = page === 'mushrooms' ? '#/focus/mushrooms' : '#/focus/plants';
  const resultsHtml = `<section class="panel workspace-pane results-pane-card"><div class="result-header compact-result-header"><div><h3>${escapeHtml(title)} results</h3><p class="results-meta">${records.length} match${records.length === 1 ? '' : 'es'}</p></div><div class="result-actions">${showSeasonButton(page) ? `<a class="buttonish" href="${inSeasonHref}">${ACTIVE_NOW_LABEL}</a>` : ''}<a class="buttonish" href="#/review">Needs Review</a></div></div><div class="result-list">${renderResultList(records, context)}</div></section>`;
  return `<section class="workspace-shell"><article class="panel workspace-head"><div><p class="eyebrow subtle">${escapeHtml(title)}</p><h2>${escapeHtml(title)}</h2><p>${escapeHtml(intro)}</p></div><div class="workspace-tabs" role="tablist" aria-label="${escapeHtml(title)} panels"><button class="workspace-tab ${paneMode === 'filters' ? 'active' : ''}" type="button" data-action="set-pane-mode" data-page="${escapeHtml(page)}" data-pane-mode="filters" aria-pressed="${paneMode === 'filters' ? 'true' : 'false'}">Filters</button><button class="workspace-tab ${paneMode === 'results' ? 'active' : ''}" type="button" data-action="set-pane-mode" data-page="${escapeHtml(page)}" data-pane-mode="results" aria-pressed="${paneMode === 'results' ? 'true' : 'false'}">Results</button></div></article>${paneMode === 'filters' ? filterHtml : resultsHtml}</section>`;
}
function renderSectionNav(activePage) { const items = [['home','Home'],['plants','Plants'],['mushrooms','Mushrooms'],['medicinal','Medicinal'],['lookalikes','Avoid These'],['timeline','Timeline'],['review','Needs Review'],['credits','Credits']]; return `<div class="section-chip-row" aria-label="Section chooser">${items.map(([slug, label]) => `<a class="section-chip ${activePage === slug ? 'active' : ''}" href="#/${slug}">${escapeHtml(label)}</a>`).join('')}</div>`; }
function renderSeasonHero(allRecords, activePage) {
  ...
  return `<section class="focus-now-panel panel"><div class="focus-now-grid"><div class="focus-now-copy"><p class="eyebrow subtle">In season now</p><h2>${escapeHtml(currentMonth)}</h2><p>${activeNow.length} entries line up with the current seasonal window. These links jump to month-filtered edible-foraging results.</p></div><div class="focus-stat-strip" aria-label="Current month summary"><div><strong>${activeNow.length}</strong><span>in season</span></div><div><strong>${plantCount}</strong><span>plants</span></div><div><strong>${mushroomCount}</strong><span>mushrooms</span></div><div><strong>${medicinalCount}</strong><span>medicinal</span></div><div><strong>${avoidCount}</strong><span>avoid</span></div><div><strong>${reviewCount}</strong><span>needs review</span></div></div></div><div class="section-chip-row" aria-label="Seasonal section chooser">${heroLinks.map(([slug,label]) => `<a class="section-chip ${activePage === slug ? 'active' : ''}" href="#/focus/${slug}">${escapeHtml(label)}</a>`).join('')}</div>${renderSectionNav(activePage)}</section>`;
}
...
export function renderDashboard({ page, allRecords, currentRecords, filters, selectedMonth, selectedWeek, paneMode, overridePayload }) {
  ...
  if (activePage === 'home') {
    return renderWorkspace({ page: 'home', title: 'Home', intro: 'This is the working page now. Use filters when you need to narrow the field, then flip back to results and keep moving.', records: currentRecords, filters, context: 'general', paneMode, scopeRecords: allRecords, extraFilters: [] });
  }
  let workspaceHtml = '';
  if (activePage === 'plants') {
    const source = allRecords.filter(isPlant);
    workspaceHtml = renderWorkspace({ page: 'plants', title: 'Plants', intro: 'Leaf shape, habitat, size, taste, and Latin names do the heavy lifting here without wasting half the screen.', records: currentRecords, filters, context: 'general', paneMode, scopeRecords: source, extraFilters: [`<label><span>Habitat</span><select data-filter="habitat">${optionHtml(vocabLabels(VOCAB.common.habitats), filters.habitat, 'Any habitat')}</select></label>`,`<label><span>Part seen</span><select data-filter="part">${optionHtml(vocabLabels(VOCAB.common.observedParts), filters.part, 'Any part')}</select></label>`,`<label><span>Size</span><select data-filter="size">${optionHtml(vocabLabels(VOCAB.common.sizes), filters.size, 'Any size')}</select></label>`,`<label><span>Taste</span><select data-filter="taste">${optionHtml(vocabLabels(VOCAB.common.tastes), filters.taste, 'Any taste')}</select></label>`]});
  ...
  if (activePage === 'review') {
    ...
    workspaceHtml = renderWorkspace({ page: 'review', title: 'Needs Review', intro: 'Timing assumptions, habitat gaps, missing images, and medicinal cleanup all land here in one queue.', records: currentRecords, filters, context: 'review', paneMode, scopeRecords: source, extraFilters: [`<label><span>Review reason</span><select data-filter="reviewReason">${optionHtml(reviewReasons, filters.reviewReason, 'Any review reason')}</select></label>`]});
    return workspaceHtml;
  }
  ...
  if (activePage === 'credits') {
    return renderCreditsPage(allRecords, overridePayload);
  }
  return workspaceHtml;
}
