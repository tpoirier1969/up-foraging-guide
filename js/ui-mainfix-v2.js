import { APP_VERSION } from "./constants-mainfix.js?v=v2.13.3-rare-recursionfix";
...
export function openDetail(slug) {
  ...
  if (!record) return;
  openHtmlModal(renderDetail(record));
}
...
export function wireModal() {
  ...
  document.addEventListener("keydown", event => {
    if (event.key === "Escape" && els.detailModal?.open) closeDetail();
  });
}
