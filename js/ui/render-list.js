...
function makeMeta(record) {
  const bits = [];
  if (record.foraging_class || record.category) bits.push(`<span class="tag">${esc(String(record.foraging_class || record.category).replaceAll("_", " "))}</span>`);
...
export function filterRecords(records, route, filtersOrSearch = "") {
  ...
  return (records || []).filter(record => {
    ...
    const { isPlant, isMushroom, medicinal, caution, edible, otherUses } = classifyRecord(record);
    if (route === "plants" && !(isPlant && edible)) return false;
    if (route === "mushrooms-gilled" && !(isMushroom && edible && record.lane === "gilled")) return false;
    if (route === "boletes" && !(isMushroom && edible && record.lane === "bolete")) return false;
    if (route === "mushrooms-other" && !(isMushroom && edible && record.lane === "other")) return false;
    if (route === "medicinal") {
      if (!medicinal) return false;
      if (!matchesMedicinalFilters(record, filters)) return false;
    }
    if (route === "lookalikes" && !caution) return false;
    if (route === "other-uses" && !otherUses) return false;
