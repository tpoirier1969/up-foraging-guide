import { loadLocalData, loadSupabaseData, loadOverridePayload } from "./api-mainfix4.js?v=v3.1.3";
import { loadLocalDataWithMaster } from "./api-masterlist.js?v=v3.1.3";
import { sortRecords, normalizeRecord, isPlant, isForagingMushroom, medicinalRecords, reviewRecords, avoidRecords } from "./data-model-mainfix4.js?v=v3.1.3";
...
const COMMONNESS_ORDER = {
  "very common": 5, "abundant": 5, "plentiful": 5, "widespread": 5,
  "common": 4, "fairly common": 4, "locally common": 4, "frequent": 4, "regular": 4,
  "occasional": 3, "scattered": 3, "moderately common": 3,
  "uncommon": 2, "infrequent": 2, "scarce": 2, "local": 2,
...
const filterState = {
  home:emptyFilter('home'), search:emptyFilter(), plants:emptyFilter(), mushrooms:emptyFilter(),
  'mushrooms-gilled':emptyFilter(), boletes:emptyFilter(), 'mushrooms-other':emptyFilter(),
  medicinal:emptyFilter(), rare:emptyFilter(), lookalikes:emptyFilter(), timeline:emptyFilter(),
  review:emptyFilter(), credits:emptyFilter(), references:emptyFilter()
};
...
function rankCommonness(record){
  const numeric = Number(record.commonness_score ?? record.commonnessScore ?? record.commonality_score ?? record.commonalityScore ?? 0);
  if (Number.isFinite(numeric) && numeric > 0) return numeric;
  const text = normSortText(record.commonness || record.commonness_label || record.commonnessLabel || record.commonality || record.commonality_label || record.commonalityLabel || '');
  if (!text) return 0;
  if (COMMONNESS_ORDER[text]) return COMMONNESS_ORDER[text];
  if (/(very common|abundant|widespread|plentiful)/.test(text)) return 5;
  if (/(common|frequent|regular|locally common|fairly common)/.test(text)) return 4;
  if (/(occasional|scattered|moderate)/.test(text)) return 3;
...
function normalizeUnderside(record){
  return (record.underside || record.mushroom_profile?.underside || []).map(v => String(v).toLowerCase());
}
function mushroomLane(record){
  const underside = normalizeUnderside(record).join(' | ');
  const slug = String(record.slug || '').toLowerCase();
  if (/(true gills|\bgills\b)/.test(underside) && !/(false gills|ridges|teeth|spines)/.test(underside)) return 'gills';
  if (/(pores|tube pores|sponge|bolete)/.test(underside)) return 'sponge';
  if (slug.includes('morel') || /(false gills|ridges|teeth|spines|smooth|jelly|coral|puffball|polypore)/.test(underside)) return 'other';
  return 'other';
}
function filteredForPage(page){
  ...
  else if(page==='mushrooms-gilled') records = state.allRecords.filter(isForagingMushroom).filter((r)=>mushroomLane(r)==='gills').filter((record)=>queryMatches(record, filterState['mushrooms-gilled']));
  else if(page==='boletes') records = state.allRecords.filter(isForagingMushroom).filter((r)=>mushroomLane(r)==='sponge').filter((record)=>queryMatches(record, filterState.boletes));
  else if(page==='mushrooms-other') records = state.allRecords.filter(isForagingMushroom).filter((r)=>mushroomLane(r)==='other').filter((record)=>queryMatches(record, filterState['mushrooms-other']));
...
function routeNavKey(activePage){
  return activePage.startsWith('mushrooms') || activePage === 'boletes' ? 'mushrooms' : activePage;
}
function renderCurrentRoute(){
  ...
  markActiveNav(routeNavKey(activePage));
  ...
  bindSharedActions({
    onFilterChange:event=>{
      const target=event.currentTarget; const page=state.route; if(!filterState[page]) return;
      filterState[page][target.dataset.filter]=target.value;
      if(target.dataset.filter==='treeType') filterState[page].hostTree='';
      renderCurrentRoute();
    },
    onClearFilters:()=>{
      const page=state.route; if(!filterState[page]) return; filterState[page]=emptyFilter(page); renderCurrentRoute();
    },
    onTimelineMonthChange:month=>{ if(!month) return; selectedTimelineMonth=month; renderCurrentRoute(); },
    onPaneModeChange:()=>{},
    onTimelineShift:direction=>{ const index=MONTHS.indexOf(selectedTimelineMonth); if(index<0) return; const delta=direction==='prev'?-1:1; selectedTimelineMonth=MONTHS[(index+delta+MONTHS.length)%MONTHS.length]; renderCurrentRoute(); },
    onToggleInSeason:page=>{ if(!filterState[page]) return; filterState[page].month=filterState[page].month===CURRENT_MONTH?'':CURRENT_MONTH; renderCurrentRoute(); }
  });
  document.querySelectorAll('[data-action="approve-review"]').forEach(btn => {
    btn.addEventListener('click', () => {
      const slug = btn.dataset.slug;
      const record = state.allRecords.find(r => r.slug === slug);
      if (!record) return;
...
function cleanRecordForUI(record){
  const images = Array.isArray(record.images) ? record.images.filter(Boolean) : [];
  const reasons = Array.isArray(record.reviewReasons) ? record.reviewReasons : [];
  const filteredReasons = images.length
    ? reasons.filter((reason) => !/image/i.test(String(reason)) && !/import/i.test(String(reason)))
    : reasons;
  return { ...record, images, reviewReasons: filteredReasons };
}
async function init(){
  wireModal();
  try{
    ...
    state.allRecords=sortRecords((payload.records||[]).map(normalizeRecord)).map(cleanRecordForUI);
