import { APP_VERSION, TABLE_NAME } from "./constants-wm4.js?v=v2.0-wm4";

async function loadOverrides() {
  try {
    const response = await fetch("data/wikimedia-image-overrides-wm4.json", { cache: "no-store" });
    if (!response.ok) throw new Error(`Override load failed: ${response.status}`);
    return await response.json();
  } catch (error) {
    console.info("Wikimedia override layer unavailable for this run:", error?.message || error);
    return { overrides: {}, metadata: { version: "none", source: "none" } };
  }
}
...
export async function loadSupabaseData() {
  ...
  const payload = {
    metadata: { project: "Upper Michigan Foraging Guide", version: APP_VERSION, source: "Supabase + local reference merge + Wikimedia override layer" },
  ...
  return applyOverrides(payload, await loadOverrides());
}

export async function loadOverridePayload() { return loadOverrides(); }
