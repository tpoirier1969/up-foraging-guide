export async function loadLocalDataWithMasterV4(baseLoadLocalData){
  const base = await baseLoadLocalData();
  try{
    const [core, plants, mushrooms] = await Promise.all([
    ...
    [...(core.records||[]),...(plants.records||[]),...(mushrooms.records||[])].forEach(r=>{
