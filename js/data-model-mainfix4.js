import { PLANT_CATEGORIES } from "./constants-mainfix.js?v=v2.1-mainfix14";
...
export function isForagingMushroom(record) { if (!isMushroom(record)) return false; const status = edibleStatus(record); const severity = severityText(record); if (FORAGING_MUSHROOM_STATUSES.has(status)) return true; if (AVOID_MUSHROOM_STATUSES.has(status)) return false; if (severity) { if (/(deadly|potentially deadly|inedible|non-culinary|toxic|questionable)/.test(severity)) return false; if (/poison/.test(severity) && !/edible|choice/.test(severity)) return false; if (/edible|choice/.test(severity)) return true; } return true; }
...
export function avoidRecords(records) { return records.filter(record => { if (isMushroom(record) && !isForagingMushroom(record)) return true; if (!isMushroom(record) && !!record.non_edible_severity) return true; return (record.look_alikes || []).length > 0; }); }
