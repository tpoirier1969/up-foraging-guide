import { APP_VERSION } from "./constants-wm1.js";
...
export function wireModal() {
  ...
  els.detailModal.addEventListener("click", event => {
    ...
    const clickedInside = rect.top <= event.clientY && event.clientY <= rect.top + rect.height && rect.left <= event.clientX && event.clientY <= rect.top + rect.height && rect.left <= event.clientX && event.clientX <= rect.left + rect.width;
